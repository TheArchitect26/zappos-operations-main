      ...
      {warehouseOrder ? (
        <Card className="border-white/10 bg-slate-900/70 p-5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-white">Warehouse readiness</h3>
          </div>
          <p className="mt-2 text-sm text-slate-400">
            {warehouseOrder.ready_for_dispatch
              ? "Your shipment has completed warehouse readiness checks and is ready for dispatch."
              : "Your shipment is being prepared for dispatch. Internal warehouse details are not exposed."}
          </p>
          {warehouseOrder.proof_of_loading_available ? (
            <p className="mt-2 text-xs text-emerald-300">
              Proof of loading has been approved for this shipment.
            </p>
          ) : null}
        </Card>
      ) : null}
