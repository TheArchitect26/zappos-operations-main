-- Append-only correction: use an unambiguous local identifier throughout the
-- atomic workspace bootstrap function.
...
BEGIN
  ...
  SELECT m.company_id INTO new_company_id
  ...
  IF new_company_id IS NOT NULL THEN
    ...
    WHERE r.company_id = new_company_id AND r.user_id = caller
